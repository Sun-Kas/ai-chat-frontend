...
import logoImage from '@/assets/image/头像.png' // Import logo image
import userAvatarImage from '@/assets/image/用户头像.png' // Import user avatar image
...
const showUserDropdown = ref(false) // New data property for dropdown visibility
...
const chatId = ref('') // Moved to a more consistent position
...
function sendMessage() {
  ...
  let url = `http://localhost:8123/api/chat/manus?message=${encodeURIComponent(userMsg.content)}`
...
function loadChatHistory() {
  const history = localStorage.getItem('agentChatHistory')
  if (history) {
...
function saveCurrentChat() {
  if (messages.value.length > 0) {
    ...
    localStorage.setItem('agentChatHistory', JSON.stringify(chatHistory.value))
  }
}
...
function goToRegister() {
  ...
  router.push('/') // Navigate to register page (changed to home page as per chatpage modification)
  showUserDropdown.value = false
}
</script>
